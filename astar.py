...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        self.f_score[x_init] = self.distance(x_init, self.x_goal)
    ...
    def get_neighbors(self, x):
        neighbors = []
        neighbors.append((x[0]-self.resolution, x[1]+self.resolution)) # (neighbor 0)
        neighbors.append((x[0], x[1]+self.resolution)) # (neighbor 1)
        neighbors.append((x[0]+self.resolution, x[1]+self.resolution))
        neighbors.append((x[0]-self.resolution, x[1]))
        neighbors.append((x[0], x[1]))
        neighbors.append((x[0]+self.resolution, x[1]))
        neighbors.append((x[0]-self.resolution, x[1]-self.resolution))
        neighbors.append((x[0], x[1]-self.resolution))
        neighbors.append((x[0]+self.resolution, x[1]-self.resolution)) # (neighbor 7)
    ...
    def solve(self):
        while len(self.open_set)>0:
            # get the state in the open set with the lowest F score:
            ...
            x_current_index = self.open_set.index(x_current)
            del self.open_set[x_current_index]
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    # x_neigh has already been visited, continue with the next
                    # neighbor:
                    continue

                # get the shortest distance from x_init to x_neigh, passing
                # through x_current:
                tentative_g_score = self.g_score[x_current] +\
                            self.distance(x_current, x_neigh)
                ...
                self.f_score[x_neigh] = tentative_g_score +\
